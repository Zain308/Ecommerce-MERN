...
export const Header = ({ activeHeading }) => {
  ...
  return (
    <>
        ...
        <div className="hidden 800px:h-[50px] 800px:my-[20px] 800px:flex items-center justify-between">
          ...
          <div className="w-[50%] relative">
            <input
              ...
              className="h-[40px] w-full px-2 border-[#3957db] border-[2px] rounded-md focus:outline-none"
            />
      ...
      {/* Mobile Header */}
      <div className={`${active === true ? "shadow-sm fixed top-0 left-0 z-10" : null} w-full h-[60px] bg-[#fff] z-50 800px:hidden`}>
        <div className="w-full flex items-center justify-between px-3 h-full">
          <BiMenuAltLeft size={40} className="cursor-pointer" onClick={() => setOpen(true)} />
          <Link to="/">
            <img src="https://shopo.quomodothemes.website/assets/images/logo.svg" alt="Logo" className="h-[40px]" />
          </Link>
          ...
          <div className="fixed w-full bg-[#0000005f] z-20 h-full top-0 left-0">
            <div className="fixed w-[70%] bg-[#fff] h-screen top-0 left-0 z-10 overflow-y-scroll">
              <div className="w-full justify-between flex pr-3 pt-3">
                <div className="relative mr-[15px] cursor-pointer" onClick={() => setOpenWishlist(true) || setOpen(false)}>
              ...
              <div className="my-8 w-[92%] m-auto h-[40px] relative">
                ...
                {searchData && searchData.length !== 0 && (
                  <div className="absolute bg-[#fff] z-10 shadow w-full left-0 p-3">
                    {searchData.map((i, index) => (
                      <Link to={`/product/${i.name.replace(/\s+/g, "-")}`} key={index} onClick={() => setOpen(false)}>
                        <div className="flex items-center py-3">
                          <img src={i.image_Url[0].url} alt="" className="w-[40px] mr-2" />
                          <h1 className="text-sm">{i.name}</h1>
                        </div>
                      </Link>
                    ))}
              ...
              <div className={`${styles.button} ml-4 !rounded-[10px] mt-4`}>
                <Link to="/shop-create">
                  <h1 className="text-[#fff] flex items-center">
                    Become Seller <IoIosArrowForward className="ml-1" />
                  </h1>
              ...
              <div className="flex w-full justify-center py-10">
                {isAuthenticated ? (
                  <Link to="/profile" onClick={() => setOpen(false)}>
                    <img src={`${backend_url}${user?.avatar}`} alt="Profile" className="w-[60px] h-[60px] rounded-full border-[3px] border-[#0eae88]" />
                  </Link>
                ) : (
                  <div className="flex items-center gap-2">
                    <Link to="/login" className="text-[18px] text-[#000000b7] font-[500]" onClick={() => setOpen(false)}>Login /</Link>
                    <Link to="/sign-up" className="text-[18px] text-[#000000b7] font-[500]" onClick={() => setOpen(false)}>Sign up</Link>
                  </div>
                )}
              </div>
            </div>
