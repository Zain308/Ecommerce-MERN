            ...
            <div className="block w-full 800px:flex">
              ...
              <div className="w-full 800px:w-[50%] pt-5 pl-[5px] pr-[5px]">
                ...
                <div className="flex justify-between items-center mt-12 pr-3">
                  <div>
                    <button className="bg-gradient-to-r from-teal-400 to-teal-500 text-white font-bold rounded-l px-4 py-2" onClick={decrementCount}>-</button>
                    <span className="bg-gray-200 text-gray-800 font-medium px-4 py-[11px]">{count}</span>
                    <button className="bg-gradient-to-r from-teal-400 to-teal-500 text-white font-bold rounded-r px-4 py-2" onClick={incrementCount}>+</button>
                  </div>
                  <div>
                    {click ? (
                      <AiFillHeart size={30} className='cursor-pointer' color="red" onClick={() => setClick(!click)} />
                    ) : (
                      <AiOutlineHeart size={30} className='cursor-pointer' color="#333" onClick={() => setClick(!click)} />
                    )}
                  </div>
                </div>
                <div className={`${styles.button} mt-6 rounded-[4px] h-11 flex items-center cursor-pointer`}>
                  <span className="text-[#fff] flex items-center">
